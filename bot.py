...
# Шаблон персоналізованого VIP-посилання
...
def init_db():
    try:
        ...
        # Міграції (додавання колонок, якщо база стара)
        ...
        conn.commit()
        conn.close()
        print(f"💾 База даних синхронізована.", flush=True)
    except Exception as e:
        print(f"❌ Помилка бази: {e}", flush=True)

# --- 4. ЛОГІКА КОНТЕНТУ ---
def get_compatibility(sign_key):
    """Генерація стабільної сумісності на день"""
...
def cmd_start(m):
    ...
    if not user_exists:
        ...
        conn.commit()
        if referrer_id:
            try: bot.send_message(referrer_id, f"🎉 Вітаємо! Новий користувач приєднався за вашим посиланням. Це зараховано для VIP!")
            except: pass
    else:
...
def central_handler(m):
    ...
    # 2. Мої підписки (UA/RU підтримка)
    ...
    if "vip" in text.lower() or "статус" in text.lower() or "друзі" in text.lower():
        ...
        if count >= 3 or is_admin:
            ...
            bot.send_message(m.chat.id, f"💎 Запросіть ще {3 - count} друзів для VIP-статусу!\n\n🔗 Твоє посилання для запрошення:\n<code>{ref_link}</code>")
        return

    # 4. Відписка від всього
...
# --- 8. РОЗСИЛКА (Щодня о 09:00 Київ) ---
def newsletter_thread():
    while True:
        try:
            now = datetime.datetime.now()
            # 07:00 UTC = 09:00 за Києвом взимку
            if now.hour == 7:
                ...
                if to_send:
                    for uid, skey in to_send:
                        try:
                            if is_sunday:
                                text = f"📅 <b>ЧАС ПЛАНУВАТИ ТИЖДЕНЬ!</b>\n\nВеликий прогноз для знака {SIGNS[skey]['ua']} вже доступний на нашому сайті."
                                kb = types.InlineKeyboardMarkup()
                                kb.add(types.InlineKeyboardButton("✨ Дивитись прогноз", url="https://www.citykey.com.ua/weekly-horoscope/"))
                            else:
                                txt = fetch_horo(skey)
...
# --- 9. ЗАПУСК З ЗАХИСТОМ ВІД ЗБОЇВ МЕРЕЖІ ---
if __name__ == "__main__":
    # Пауза 20 сек для ініціалізації мережі Railway
    ...
    while True:
        try:
            ...
            # Якщо мережа "відвалилася" (Errno 101), бот просто чекає 15 сек і пробує знову
